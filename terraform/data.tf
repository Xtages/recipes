...
data "template_file" "app_task_definition" {
  ...
  vars = {
    ...
    APP_VCPU             = var.PLAN_PAID ? lookup(local.xtages_backends[var.ENV], "app_td_vcpu") : lookup(local.free_plan, "app_td_vcpu")
    APP_MEM              = var.PLAN_PAID ? lookup(local.xtages_backends[var.ENV], "app_td_mem") : lookup(local.free_plan, "app_td_mem")
    NGINX_MEM            = var.PLAN_PAID ? lookup(local.xtages_backends[var.ENV], "nginx_td_mem") : lookup(local.free_plan, "nginx_td_mem")
    NGINX_VCPU           = var.PLAN_PAID ? lookup(local.xtages_backends[var.ENV], "nginx_td_vcpu") : lookup(local.free_plan, "nginx_td_vcpu")
  }
}
